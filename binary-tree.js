...
class BinaryTree {
  ...
  minDepth() {
    ...
    function minDepthHelper(node) {
      ...
      if (node.left) lDepth = minDepthHelper(node.left);
      if (node.right) rDepth = minDepthHelper(node.right);
  ...
  maxDepth() {
    ...
    function maxDepthHelper(node) {
      let lDepth, rDepth;
      if (node.left === null && node.right === null) return 1;
      if (node.left) lDepth = maxDepthHelper(node.left);
      if (node.right) rDepth = maxDepthHelper(node.right);
  ...
  nextLarger(lowerBound) {
    ...
    while (queue.length) {
      ...
      let reassignJustAbove = currentVal < justAbove || justAbove === null;

      if (higherThanLowerBound && reassignJustAbove) {
        justAbove = currentVal;
      }
